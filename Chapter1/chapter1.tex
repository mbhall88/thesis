...
We use a simulated genome and associated \ont{} dataset to explore the impact of various parameters on the \denovo{} discovery method and find read depth to have a vital influence.
...
After quasi-mapping the sample reads to the \panrg{} and inferring the maximum likelihood path (a path of minimizing \kmer{}s; $K_n$) for each locus \prg{}, $n$ - as explained previously in \autoref{sec:pandora-map-intro} - we discover candidate regions in this path. To do so, we transform $K_n$ into a sequence, $M_n$, and use the \kmer{} coverage in $K_n$ as an approximation to the read coverage of each base in $M_n$.

We define a candidate region, $r$, as an interval within $M_n$ where read depth (coverage) is less than a given threshold for less than $m$ consecutive positions. The coverage for each position is calculated as the maximum \kmer{} depth of all \kmer{}s overlapping it. Thus, $m$ acts to restrict the size of variants we can detect. If set too large, the following steps become much slower due to the combinatorial expansion of possible paths. 
...
In addition to $T_r$, obtained by performing DFS on $G_r$, we produce a distance map, $D_r$, that results from running reversed breadth-first search (BFS) on $G_r$, beginning from the \emph{end} anchor ($a_R$). We say reversed BFS as we explore the \emph{predecessors} of each node, rather than the successors. $D_r$ maps each \kmer{} in $G_r$, that is reachable from $a_R$ via reversed BFS, to an integer that describes the shortest path from that \kmer{} to $a_R$.
...
\noindent
In the end, for each candidate region ($r$), we are left with a collection of paths ($V_r$) between two \kmer{}s ($a_L$ and $a_R$). We create the final candidate paths by replacing the sequence between $a_L$ and $a_R$ in the maximum likelihood path ($M_n$) with each path ($p_r$) in $V_r$. These are written to file - with one file per candidate region. Padding the candidate paths in this way ensures they are inserted into the \prg{} in the correct location (see \autoref{sec:denovo-insert}). 
...
Previous work from Wick \etal{} has shown that for Enterobacteriaceae, the majority of \ont{} sequencing errors are related to Dcm methylation sites \cite{wick2019}. In version 3.2.1 of \guppy{} (the ONT-provided basecalling software) a new \emph{methylation-aware} model was made available. As \ecoli{} is a member of this family, we set out to test a subset of 4 samples from our dataset to see whether a methylation-aware model indeed has a noticeable impact on the precision and recall from \pandora{} - with and without \denovo{} variant discovery.
