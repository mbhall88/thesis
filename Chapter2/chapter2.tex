...
cluster in the Nanopore clustering. An illustrated, worked example of these metrics can be found in \autoref{sec:cluster-example}.

\subsection{Evaluating \ont{} transmission clusters: bcftools}

For the four Illumina SNP distance thresholds of interest - 0, 2, 5, and 12 - the corresponding bcftools thresholds we use are 0, 2, 5, and 11. We chose to forego the model-based predicted thresholds and instead use the hand-picked ones based on the analysis in \autoref{sec:dist-sweep}.
...
12        & 1.0  & 0.845 & 0.031 (3/97) 
\end{tabular}
\caption{Summary of bcftools clustering metrics for four SNP distance thresholds. The fractions in parentheses for XCR indicate the underlying numbers. SACR=sample-averaged cluster recall; SACP=sample-averaged cluster precision; XCR=excess clustering rate.}
\label{tab:bcftools-cluster-summary}
\end{table}

\subsection{Evaluating \ont{} transmission clusters: \pandora{} single-sample approach}

\subsection{Evaluating \ont{} transmission clusters: \pandora{} multi-sample approach}
...
% dense prg preformance is worse than sparse mostly because of shared kmers leading to coverage on alt alleles in longer alleles and especially for sites with multiple alleles
